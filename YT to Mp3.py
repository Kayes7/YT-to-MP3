...
n=5 #Quantity of videos taking into account

def get_duration(video_data_duration): #Converts the str duration (mm:ss) to int
...
def best_option(name,artist,results): #Return the index number of the best option considering the name, artist, 'Audio' into the title and the duration (only audio videos tend to have less duration)
    n=len(results)
    i=0
    best=0
...
def main():
    ...
    while not quit:
        ...
        print()
        downloader(path)
        cprint("Saved file in "+path,"dark_grey")
        ans=input("Another download? (Y/N): ")
